...
class PHCA:
    ...
    def predict(self, X_test):
        ...
        for x_new in tqdm(X_test):
            ...
            self.selected_class = class_selector(self.PD_byClass, self.PD_byClass_plus, self.dim)
...
def get_persistence_diagram(data, dim:list):
    return {key: ripser.ripser(np.array(data[key]), maxdim=max(dim))['dgms'] for key in data.keys()}
...
def get_lifespan(PD:list, dim:list):
    total_lifespan = 0
    for d in dim:
...
def class_selector(orig_PD:dict, new_PD:dict, dim:list):
    scores = {}
